...
export default function Hero() {
  ...
  return (
    <section className="relative overflow-hidden min-h-screen">
      ...
      {/* Ana içerik: Sol yazı + Sağ konuşmacılar */}
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-16 pb-20 flex flex-col lg:flex-row items-center lg:items-center gap-12 lg:gap-8 min-h-[calc(100vh-80px)]">
        ...
        >
          <span className="inline-block bg-[#e31e26]/15 text-white text-sm font-semibold px-5 py-2 rounded-full mb-8 border border-[#e31e26]/40">
            {webinarData.badge}
          </span>

          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight mb-3 text-white">
            {webinarData.title}
          </h1>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight mb-6 text-[#e31e26]">
            {webinarData.titleHighlight}
          </h2>
          ...
          <div className="flex items-center gap-6 text-gray-400 mb-10 text-sm sm:text-base">
        ...
        {/* Sağ taraf: Konuşmacılar */}
        <motion.div
          className="lg:w-[45%] flex gap-4 justify-center lg:justify-end"
          initial={{ opacity: 0, x: 40 }}
          animate={{ opacity: 1, x: 0 }}
        ...
        >
          {webinarData.speakers.map((speaker, index) => (
            <div
              key={index}
              className="relative group flex flex-col w-[200px] sm:w-[220px]"
            >
              {/* İsim ve title - üstte */}
              ...
              {/* Fotoğraf alanı - büyük dikey kart */}
              <div className="relative overflow-hidden rounded-2xl border-2 border-white/10 group-hover:border-[#e31e26]/40 transition-all duration-500 aspect-[3/4] bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-sm">
                {/* Placeholder - gerçek fotoğraf eklenecek */}
              ...
              {/* Kart altında kırmızı accent çizgi */}
